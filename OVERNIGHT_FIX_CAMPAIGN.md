...
- ★ #70 CHAMFER-CROSSES-FILLET (the defect B1 exposed; taken on as a dedicated task) — 2 real defects (2 deep investigations):
  ...
  - DEFECT 2 (faces 3/5 self-overlap) = the deeper #72, NOT fixed. On a cube face bordering two corner-blended edges, the two inset trim tracks run FULL-span and cross in a bowtie at their intersection (e.g. (4,5,4)). self_overlapping_planar_faces is CORRECT (real defect). Canonical pinned test: harness/integration.rs:562 chamfer_crossing_fillet_self_overlap_pinned_70 (#[ignore]).
    ...
    - RE-SCOPED PATH (3 deliverables, each gated): #1 a DEGREE-2 POST-SURGERY apex/junction solver (post-chamfer bevel face + 2 fillet cylinders, not 3 edges; ~120 lines reusing predict_corner_blend_axis/least_squares_concurrent_point; unit-test J/P=live rim corners) — ✅ DONE (2bfd8aa): solve_corner_junctions_post_surgery + reconstruct_bevel_plane (RuledSurface→Plane::from_three_points) + fillet_cap_spine (live fillet face is CylindricalFillet not Cylinder → spine+radius). Test reproduces apex (4,4,4) + J/P = live rim corners (5,5,4)/(4,5,5)/(5,4,5) exactly; returns Ok on the real degree-2 input. blend_graph 22/22, no regression.
    ★★ CRITICAL FINDING (#2+#3 attempt, 2026-06-22 — REDIRECTS the approach + caught a mis-scope):
    - The 1C2F cap is ALREADY topologically closed + 2-manifold (0 boundary/non-manifold B-Rep edges). The defect is PURELY geometric self-overlap on the planar cube faces (the "euler=-3/boundary_edges=90" was a TESSELLATION-mesh metric, not a B-Rep hole). The cap triangle is ALREADY built at deliverable #1's junctions.
    - THE TRAP: those junctions (p12=(5,4,5), j1=(5,5,4), j2=(4,5,5)) lie ON the cube edges (un-retracted along the fillet axis), so each cap-rim arc lies IN a cube-face plane (edge19⊂x=5, edge22⊂z=5, edge13⊂y=5) → each rim is demanded by THREE faces (cap + fillet + cube) → inserting it into the cube loop = non-manifold; a separate coplanar straight cut-edge = degenerate sliver. IRREDUCIBLE while junctions sit on the cube edges.
    - WHY ALL-FILLET IS WATERTIGHT: the gold-standard all-fillet 3-corner AXIALLY RETRACTS each fillet V-cap arc OFF the cube planes (to x=4 etc., setback=1.0) to an inner apex triangle (corners off the cube edges, e.g. (5,4,4)); the arcs are shared fillet∧sphere, never touch the cube planes → the cube faces close with straight cut edges.
    - ★ MIS-SCOPE (mine): I gated deliverable #1 on "J/P = live rim corners" — but the live rim corners ARE the broken bowtie positions, so the test validated REPRODUCING THE BROKEN STATE. #1's apex=(4,4,4)+setback=1.0 are the RIGHT tool; its junctions j1/j2/p12 are a RED HERRING for capping.
    - ★ CORRECTED PATH: generalize apply_triangular_nurbs_corner (the WORKING all-fillet apex-retraction: trim_cap_arc_in_place + reanchor_seam_edges_at_cap_arc_endpoint, axial-retract by setback) to the 1C2F case = ONE chamfer-bevel rail + TWO cylinder-fillet rails; drive the cap from apex + setback_fillet_* (NOT the junctions); cap the retracted INNER triangle (off the cube planes); close the cube faces with straight cut edges (now manifold cube∧blend). Adapts a PROVEN path — the most promising route yet. #2 apply setbacks in the fillet chain (fillet.rs:563, 1C2F-gated) → retract V-caps + open gaps — RISKS the shared rails, MUST be poke_matrix-gated. #3 excise cube-face triangles + rebuild cap on retracted rims (trim_cap_arc_in_place + reanchor + R1 chord-replace + insert_cap_into_face_loop→pub(crate)).
