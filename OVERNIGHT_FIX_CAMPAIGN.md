...
- ★ #70 CHAMFER-CROSSES-FILLET (the defect B1 exposed; taken on as a dedicated task) — 2 real defects (2 deep investigations):
  ...
  - DEFECT 2 (faces 3/5 self-overlap) = the deeper #72 JUNCTION RECONSTRUCTION, NOT fixed. On a cube face bordering two corner-blended edges, the two inset trim tracks each run the FULL span and cross in a bowtie at their intersection (e.g. (4,5,4)) — exact straight lines crossing (self_overlapping_planar_faces is CORRECT). ROOT: corner-shared splice (edge_blend_topology.rs:289-308,:547-553) leaves both tracks full-span, never trims to a shared junction. FIX-PATH: retrim each track's V-side end onto the in-plane intersection via rewire_edge_vertex (edge_blend_topology.rs:597-709) + merge a junction vertex; then un-ignore harness/integration.rs:562-603 + require self_overlapping_planar_faces==[]. HIGH-RISK (every blend corner) → dedicated focused pass.
