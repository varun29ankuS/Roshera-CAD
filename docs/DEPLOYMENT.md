...
| `ROSHERA_REQUIRE_INTENT` | `api-server/src/main.rs` (`IntentPosture::from_env`) | Intent-required mode (audit item 10 / S2, 2026-08-15 closeout). Defaults **OFF** (`IntentPosture::Optional`) — a mutating call on any of the ten gate-3 routes (boolean/shell/mirror/fillet/chamfer/transform/pattern-linear/pattern-circular/face-extrude/sketch-extrude-cut) with no `X-Roshera-Intent` header runs exactly as it always has; this is a deliberate design decision (absent-is-legal), not a gap being tolerated. Set to `1` or `true` (case-insensitive) to switch ON: the same ten routes then refuse (`422 intent_required`) any call with no declared intent, typed the same way `unsound_base` is. The partner stack **does not set it** — the frontend and every legacy REST client never send this header. Intended for an RL run or an agent-only deployment that wants to demand declared intent from every mutation. |
