...
impl Vector2 {
    ...
    ///
    /// Violating it yields `NaN` or `±∞`-tainted components that
    /// propagate silently. Use [`try_normalize`](Self::try_normalize)
    /// when the precondition cannot be proven statically.
    ///
