...
use geometry_engine::operations::{boolean_operation, BooleanOp as GeometryBooleanOp, BooleanOptions};
use geometry_engine::primitives::solid::SolidId;
use geometry_engine::tessellation::{tessellate_solid, TessellationParams};
use serde::{Deserialize, Serialize};
use serde_json::json;
use shared_types::geometry::{MaterialProperties, AnalyticalProperties};
...
async fn handle_create_primitive(
    ...
    let result = match primitive_type.to_lowercase().as_str() {
        "box" | "cube" => {
            let width = parameters.get("width").and_then(|v| v.as_f64()).unwrap_or(10.0);
            let height = parameters.get("height").and_then(|v| v.as_f64()).unwrap_or(10.0);
            let depth = parameters.get("depth").and_then(|v| v.as_f64()).unwrap_or(10.0);
            builder.create_box_3d(width, height, depth)
        },
        "sphere" => {
            let radius = parameters.get("radius").and_then(|v| v.as_f64()).unwrap_or(5.0);
    ...
    match result {
        Ok(geometry_id) => {
            ...
            let analytical_geometry = shared_types::AnalyticalGeometry {
                ...
                parameters: parameters.as_object()
...
async fn handle_boolean_operation(
    ...
    let geometry_bool_op = match operation.as_str() {
        ...
        _ => return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, format!("Unknown boolean operation: {}", operation)))),
    };
    
    // Perform boolean operation using core geometry engine
    let options = BooleanOptions::default();
    let result = boolean_operation(&mut geometry_model, solid_a_id, solid_b_id, geometry_bool_op, options);
    ...
    match result {
        Ok(result_solid_id) => {
            ...
            let solid = geometry_model.solids.get(result_solid_id)
            ...
            let mesh = Mesh {
                vertices: triangle_mesh.vertices.iter()
                    .flat_map(|v| [v.position[0] as f32, v.position[1] as f32, v.position[2] as f32])
                    .collect(),
                normals: triangle_mesh.vertices.iter()
                    .flat_map(|v| [v.normal[0] as f32, v.normal[1] as f32, v.normal[2] as f32])
                    .collect(),
                indices: triangle_mesh.triangles.iter()
...
async fn handle_get_analytical_properties(
    ...
    if let Ok(session) = state.session_manager.get_session(session_id).await {
        ...
        if let Some(obj) = session_state.objects.get(&uuid::Uuid::parse_str(&object_id).unwrap_or_default()) {
    ...
    for chunk in mesh.indices.chunks(3) {
        if chunk.len() == 3 {
            ...
            if i2 + 2 < mesh.vertices.len() {
                let v0 = [mesh.vertices[i0] as f64, mesh.vertices[i0+1] as f64, mesh.vertices[i0+2] as f64];
                let v1 = [mesh.vertices[i1] as f64, mesh.vertices[i1+1] as f64, mesh.vertices[i1+2] as f64];
                let v2 = [mesh.vertices[i2] as f64, mesh.vertices[i2+1] as f64, mesh.vertices[i2+2] as f64];
...
fn compute_analytical_properties(
    ...
    let solid = geometry_model.solids.get(solid_id)
        .ok_or_else(|| "Solid not found for properties computation".to_string())?;
    
    // Get the shell from the solid
    let shell = geometry_model.shells.get(solid.outer_shell)
    ...
    for &face_id in &shell.faces {
        let face = geometry_model.faces.get(face_id)
            .ok_or_else(|| "Face not found".to_string())?;
        
        // Get the surface for this face
        let surface = geometry_model.surfaces.get(face.surface_id)
        ...
        match surface.surface_type() {
            SurfaceType::Plane => {
                ...
                let loop_data = geometry_model.loops.get(face.outer_loop)
                ...
                for (i, &edge_id) in loop_data.edges.iter().enumerate() {
                    let edge = geometry_model.edges.get(edge_id)
                        .ok_or_else(|| "Edge not found".to_string())?;
                    
                    let v1 = geometry_model.vertices.get(edge.start_vertex)
                        .ok_or_else(|| "Vertex not found".to_string())?;
                    let v2 = geometry_model.vertices.get(edge.end_vertex)
                        .ok_or_else(|| "Vertex not found".to_string())?;
            ...
            SurfaceType::Cylinder => {
                ...
                if let Some(cylinder) = surface.as_any().downcast_ref::<Cylinder>() {
                    ...
                    let height = cylinder.height_limits.map(|[b, t]| (t - b).abs()).unwrap_or(10.0);
            ...
            SurfaceType::Cone => {
                ...
                if let Some(cone) = surface.as_any().downcast_ref::<Cone>() {
                    ...
                    let height = cone.height_limits.map(|[b, t]| (t - b).abs()).unwrap_or(10.0);
            ...
            SurfaceType::Torus => {
                ...
                if let Some(torus) = surface.as_any().downcast_ref::<Torus>() {
                    ...
                    // Torus volume: 2π²Rr²
                    let torus_volume = 2.0 * std::f64::consts::PI.powi(2) * major_radius * minor_radius.powi(2);
                    total_volume += torus_volume;
                    
                    // Torus surface area: 4π²Rr
                    total_surface_area += 4.0 * std::f64::consts::PI.powi(2) * major_radius * minor_radius;
...
fn tessellate_solid_for_display(
    ...
    let tessellation_params = match quality {
        ...
        shared_types::DisplayQuality::Custom { max_edge_length, max_angle_deviation, chord_tolerance } => {
            TessellationParams {
                max_edge_length,
                max_angle_deviation,
                chord_tolerance,
...
fn compute_solid_bounding_box(solid: &geometry_engine::primitives::solid::Solid) -> shared_types::BoundingBox {
