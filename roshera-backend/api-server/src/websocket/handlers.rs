//! WebSocket message handlers (simplified for demo)

use crate::websocket::protocol::GeometryWSCommand;
use crate::AppState;
use axum::{
    extract::{
...
use shared_types::{PrimitiveType, CommandResult};
...
///
/// Falls back to `0` if the system clock is set before `UNIX_EPOCH`. WebSocket
                ...
                match serde_json::from_str::<ClientMessage>(&text) {
                    Ok(client_msg) => {
                        ...
                        match client_msg {
                            ...
                            ClientMessage::AICommand { command, request_id } => {
                                // Handle AI commands - convert natural language to geometry
                                match command {
                                    super::protocol::AIWSCommand::ProcessCommand { text, context } => {
                                            ...
                                            Ok(parsed_command) => {
                                                ...
                                                match parsed_command {
                                                    VoiceCommand::ActivatePartMaturityWorkflow { primitive, parameters, sketch_plane, .. } => {
                                                        // First, activate the Part Maturity workflow (Create stage)
                                                        ...
                                                        match result {
                                                            Ok((shape_name, LocalGeometryId::Solid(solid_id))) => {
                                                                ...
                                                                let response = ServerMessage::AIResponse {
                                                                    response: crate::websocket::protocol::AIResponse::CommandExecuted {
                                                    ...
                                                    _ => {
                                                        ...
                                                        let response = ServerMessage::Error {
                                                            ...
                                                            message: "This AI command is not yet implemented".to_string(),
                                                            details: None,
                                                            request_id,
                                                        };
                                                        if let Ok(json) = serde_json::to_string(&response) {
                            ...
                            ClientMessage::SessionCommand { command, request_id } => {
                                // Handle session commands
                                match command {
                                    super::protocol::SessionWSCommand::JoinSession { session_id } => {
                                        info!("Processing JoinSession command for session: {}", session_id);
                                        
                                        // Use the existing session join logic
                                        let user_name = format!("User_{}", &user_id[..8]); // Default user name
                                        
                                        // Create or join session
                                        let actual_session_id = if session_id == "default" || session_id.is_empty() {
                                        ...
                                        if let Ok(session) = state.session_manager.get_session(&actual_session_id).await {
                                            let mut session_state = session.write().await;
                                            let now = unix_millis_now();

                                            // Check if user already exists (to avoid duplicates)
                                            if !session_state.active_users.iter().any(|u| u.id == user_id) {
                                                ...
                                                info!("Added user {} to session {}", user_name, actual_session_id);
                                            }
                                        }
                                        ...
                                        if let Ok(session) = state.session_manager.get_session(&actual_session_id).await {
                                            ...
                                            let collaborators: Vec<CollaboratorInfo> = session_state
                                                ...
                                                .map(|user| CollaboratorInfo {
                                                    ...
                                                    is_active: true,
                                                    initial: user.name.chars().next().unwrap_or('U').to_string(),
                                                    color: user.color,
                                                    last_activity: user.last_activity,
                                                })
                                                .collect();

                                            let collaborators_msg = WebSocketMessage::CollaboratorsUpdate {
                                                session_id: actual_session_id.clone(),
                                                users: collaborators,
                                            };

                                            if let Ok(json) = serde_json::to_string(&collaborators_msg) {
                                                info!("Sending CollaboratorsUpdate with {} users", session_state.active_users.len());
                                                let _ = sender.send(Message::Text(json.into())).await;
                                            }
                                        }
                                    }
                                    super::protocol::SessionWSCommand::LeaveSession => {
                                        ...
                                        if let Some(session_id) = &current_session_id {
                                            ...
                                            if let Ok(session) = state.session_manager.get_session(session_id).await {
                                                let mut session_state = session.write().await;
                                                session_state.active_users.retain(|u| u.id != user_id);
                                            }
                                    ...
                                    super::protocol::SessionWSCommand::CreateSession { name, description } => {
                                    ...
                                    super::protocol::SessionWSCommand::ShareObject { object_id, permissions } => {
                                        info!("Sharing object {:?} with permissions {:?}", object_id, permissions);
                                        ...
                                        if let Some(session_id) = &current_session_id {
                                            ...
                                            let response = ServerMessage::SessionUpdate {
                                                update: super::protocol::SessionUpdate::ObjectShared {
                                            ...
                                            if let Ok(json) = serde_json::to_string(&response) {
                                                info!("Object {:?} shared with permissions {:?}", object_id, permissions);
                                                let _ = sender.send(Message::Text(json.into())).await;
                                            }
                                            ...
                                            if let Ok(session) = state.session_manager.get_session(session_id).await {
                                                ...
                                                info!("Broadcasting object share to {} users", session_state.active_users.len());
                                            }
                                        } else {
                                            let response = ServerMessage::Error {
                                                error_code: "NOT_IN_SESSION".to_string(),
                                                message: "Must be in a session to share objects".to_string(),
                                                details: None,
                                                request_id,
                                            };
                                            if let Ok(json) = serde_json::to_string(&response) {
                                    ...
                                    super::protocol::SessionWSCommand::UpdatePresence { cursor_position, selected_objects } => {
                                        ...
                                        if let Some(session_id) = &current_session_id {
                                            ...
                                            if let Ok(session) = state.session_manager.get_session(session_id).await {
                                                ...
                                                if let Some(user) = session_state.active_users.iter_mut().find(|u| u.id == user_id) {
                                                    // Update cursor position and selected objects
                                                    user.cursor_position = cursor_position;
                                                    user.selected_objects = selected_objects.clone();
                                            ...
                                            info!("Presence updated for user {} in session {}", user_id, session_id);
                                        } else {
                                            let response = ServerMessage::Error {
                                                error_code: "NOT_IN_SESSION".to_string(),
                                                message: "Must be in a session to update presence".to_string(),
                                                details: None,
                                                request_id,
                                            };
                                            if let Ok(json) = serde_json::to_string(&response) {
                            ...
                            ClientMessage::TimelineCommand { command, request_id } => {
                                ...
                                let response = match command {
                                    super::protocol::TimelineWSCommand::Undo { steps } => {
                                        ...
                                        ServerMessage::TimelineUpdate {
                                            update: super::protocol::TimelineUpdate::UndoPerformed {
                                                steps: steps_to_undo,
                                            },
                                        }
                                    }
                                    super::protocol::TimelineWSCommand::Redo { steps } => {
                                        ...
                                        // Timeline undo/redo would be implemented through event replay
                                        ServerMessage::TimelineUpdate {
                                            update: super::protocol::TimelineUpdate::RedoPerformed {
                                                steps: steps_to_redo,
                                            },
                                        }
                                    }
                                    super::protocol::TimelineWSCommand::CreateBranch { name, from_point } => {
                                        info!("Creating branch '{}' from point {:?}", name, from_point);
                                        ...
                                        let result = branch_manager.create_branch(
                                            ...
                                            timeline_engine::BranchPurpose::UserExploration {
                                                description: format!("Branch created at {}", chrono::Utc::now()),
                                            },
                                        );
                                        
                                        match result {
                                            Ok(branch_id) => {
                                                info!("Created branch '{}' with ID: {:?}", name, branch_id);
                                                ServerMessage::TimelineUpdate {
                                    ...
                                    super::protocol::TimelineWSCommand::SwitchBranch { branch_name } => {
                                        info!("Switching to branch '{}'", branch_name);
                                        
                                        // Branch switching would be handled through the timeline
                                        ServerMessage::TimelineUpdate {
                                            update: super::protocol::TimelineUpdate::BranchSwitched {
                                                from: "main".to_string(),
                                                to: branch_name.clone(),
                                            },
                                        }
                                    }
                                    super::protocol::TimelineWSCommand::MergeBranch { source, target, strategy } => {
                                        info!("Merging branch '{}' into '{}' using {:?} strategy", source, target, strategy);
                                    ...
                                    super::protocol::TimelineWSCommand::ExecuteOperation { operation } => {
                                        ...
                                        let timeline_op = timeline_engine::Operation::Generic {
                                            ...
                                            parameters: serde_json::to_value(&operation).unwrap_or(serde_json::Value::Null),
                                        };
                                        
                                        // Timeline is wrapped in Arc<RwLock>, need to write lock it
                                        let timeline = state.timeline.write().await;
                                        let result = timeline.add_operation(
                                            timeline_op,
                                            timeline_engine::Author::System,
                                        ...
                                        match result {
                                            Ok(event_id) => {
                                                info!("Operation recorded in timeline with ID: {:?}", event_id);
                                                ServerMessage::TimelineUpdate {
                            ...
                            ClientMessage::ExportCommand { command, request_id } => {
                                ...
                                let response = match command {
                                    super::protocol::ExportWSCommand::ExportSTL { object_ids, format } => {
                                        info!("Exporting {} objects to STL ({:?})", object_ids.len(), format);
                                        ...
                                        if !solids_to_export.is_empty() {
                                            ...
                                            for solid in &solids_to_export {
                                                ...
                                                if let Some(tessellated) = model.tessellate_solid(solid.id, 0.01) {
                                            ...
                                            let flat_vertices = all_vertices.iter()
                                                .flat_map(|v| vec![v[0], v[1], v[2]])
                                                .collect();
                                    ...
                                    super::protocol::ExportWSCommand::ExportOBJ { object_ids, include_materials } => {
                                        info!("Exporting {} objects to OBJ (materials: {})", object_ids.len(), include_materials);
                                    ...
                                    super::protocol::ExportWSCommand::ExportROS { filename, options } => {
                                        info!("Exporting to ROS format: {}", filename);
                                        
                                        // Get all solids from model for ROS export
                                        let all_solids: Vec<_> = model.solids.iter()
                                        ...
                                        if !all_solids.is_empty() {
                                            ...
                                            // Write to file
                                            let result = std::fs::write(&filename, export_json.to_string())
                                            ...
                                            match result {
                                                Ok(bytes_written) => {
                                                    ...
                                                    ServerMessage::ExportComplete {
                                                        result: super::protocol::ExportResult::ROS {
                                                ...
                                                Err(e) => {
                                                    ...
                                                    ServerMessage::Error {
                                                        error_code: "EXPORT_FAILED".to_string(),
                                                        message: format!("Failed to export ROS: {:?}", e),
                                                        details: None,
                                                        request_id,
                                                    }
                                                }
                            ...
                            ClientMessage::Unsubscribe { topics, request_id } => {
                                ...
                                if let Some(ref session_id) = current_session_id {
                                    if let Ok(session) = state.session_manager.get_session(session_id).await {
                                        ...
                                        if let Some(user) = session_state.active_users.iter_mut().find(|u| u.id == user_id) {
                                            info!("User {} unsubscribed from {:?}", user_id, topics);
                                        }
                                    }
                                }
                            ...
                            ClientMessage::Query { query_type, request_id } => {
                                info!("Processing query: {:?}", query_type);
                                
                                // Parse the query type from JSON and execute
                                let result = if let Ok(query) = serde_json::from_value::<super::protocol::WSQueryType>(query_type.clone()) {
                                    match query {
                                        super::protocol::WSQueryType::GetObject { object_id } => {
                                            ...
                                            if let Some(local_id) = state.get_local_id(&object_id) {
                                                ...
                                                ServerMessage::Error {
                                                    ...
                                                    message: "Object ID not found in local mapping".to_string(),
                                                    details: None,
                                                    request_id,
                                                }
                                            }
                                        }
                                        super::protocol::WSQueryType::ListObjects { filter, limit, offset } => {
                                        ...
                                        super::protocol::WSQueryType::GetSessionInfo { session_id } => {
                                        ...
                                        super::protocol::WSQueryType::GetMetrics => {
                                            ...
                                            let metrics = state.request_metrics.clone();
                                            let metric_data: Vec<_> = metrics.iter().map(|entry| {
                                                serde_json::json!({
                                                    "endpoint": entry.key(),
                                                    "count": *entry.value()
                                                })
                                        ...
                                        super::protocol::WSQueryType::SearchObjects { query, limit } => {
                                            ...
                                            let results: Vec<_> = matching_solids.iter().map(|solid| {
                                                let uuid = state.create_uuid_for_local(solid.id);
                                                serde_json::json!({
                                                    "id": uuid.to_string(),
                                                    "type": "solid",
