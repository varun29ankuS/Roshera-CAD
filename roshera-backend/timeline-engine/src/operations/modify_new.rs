...
use crate::{
    ...
    Operation, OperationOutputs, ResourceEstimate,
    Modification, EntityType,
    execution::{ExecutionContext, OperationImpl},
};
...
#[async_trait]
impl OperationImpl for ModifyOperation {
    fn can_handle(&self, operation: &Operation) -> bool {
        matches!(operation, Operation::Modify { .. })
    }
    
    fn validate(&self, operation: &Operation, context: &ExecutionContext) -> TimelineResult<()> {
        if let Operation::Modify { entity, modifications } = operation {
    ...
    async fn execute(
        ...
        if let Operation::Modify { entity, modifications } = operation {
            ...
            // Mark as modified
            if let Some(obj) = entity_state.properties.as_object_mut() {
                obj.insert("last_modified".to_string(), 
